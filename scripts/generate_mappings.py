...
Each CSV matches the format of SDTMIG_CG_to_core_mapping.csv:

    Rule ID | <ver1> | <ver2> | ... | Status | CORE-ID

- Each standard block in a rule.yml fans out to its own CSV independently.
- One row per Rule Identifier Id per standard. A single rule.yml with
  CG0415 under SDTMIG and TIG0570 under TIG produces a row in each CSV.
- Version columns are populated with the version string when the rule
  applies to that version, blank otherwise.
- Status comes from Core.Status.
- CORE-ID is Core.Id for published rules, blank for unpublished.
    ...
    for authority in authorities:
        ...
        for standard in (authority.get("Standards") or []):
            if not isinstance(standard, dict):
                continue
            std_name = str(standard.get("Name") or "").strip()
            if not std_name:
                continue
...
def collect_all_rule_files(root: Path) -> list[Path]:
    """
    Recursively find every rule.yml under Published/ and Unpublished/.
...
def _repair_unclosed_quotes(raw: str) -> tuple[str, bool]:
    """
    Scan each line for an unclosed single-quoted scalar and close it.
    A line has an unclosed single quote when it contains an odd number of
    single-quote characters that are not part of an escaped pair (\'\').

    Returns (repaired_text, was_modified).
    """
    lines = raw.splitlines(keepends=True)
    repaired = False
    for i, line in enumerate(lines):
        # Strip the newline for analysis
        stripped = line.rstrip("\n\r")
        # Count unescaped single quotes: replace escaped '' with placeholder first
        normalized = stripped.replace("''", "\x00\x00")
        quote_count = normalized.count("'")
        if quote_count % 2 == 1:
            # Odd number of single quotes — close the scalar at end of line
            lines[i] = stripped + "'" + (line[len(stripped):] or "\n")
            repaired = True
...
def write_csv(rows: list[dict], output_path: Path) -> None:
    ...
    print(f"  [OK] {output_path.name}  ({len(rows)} rules)")
...
def main() -> None:
    ...
    if not rule_files:
        print("No rule.yml files found under Published/ or Unpublished/.")
        sys.exit(0)

    print(f"Found {len(rule_files)} rule.yml file(s). Building mappings...\n")
    ...
    print(f"\nDone. CSVs written to '{mappings_dir}'.")
